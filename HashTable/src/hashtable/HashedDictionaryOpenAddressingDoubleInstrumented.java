...
//>>>>>>>>>>>>> ADDED CODE >>>>>>>>>>>>>>
    private int getSecondHashIndex(K key){
        ...
        largestPrime(hashTable.length-1);
        //int h2 = hashTable.length-1;
...
//<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
    ...
    public static void largestPrime(int num) {
        int check = 0;
...
//>>>>>>>>>>>>> MODIFIED THE FOLOWING FOR DOUBLE PROBING >>>>>>>>>>>
               index = (index + h2) % hashTable.length; // Linear probing
